/*//GEN-FIRST:event_TPassKeyPressed
  Dilarang keras menggandakan/mengcopy/menyebarkan/membajak/mendecompile//GEN-LAST:event_TPassKeyPressed
...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            pemberi_hutang_lain=false,dokter_asal_pasien_ranap=false,duta_parkir_rekap_keluar=false,surat_keterangan_layak_terbang=false,bayar_beban_hutang_lain=false,persetujuan_pernyataan_pemilihan_dpjp=false,grafik_harian_pasien_prb=false,
    ...
    private void ppSemua1ActionPerformed(java.awt.event.ActionEvent evt) {                                         
        for(i=0;i<tbUser.getRowCount();i++){
            tbUser.setValueAt(true,i,2);
        }
    }                                        

    private void ppBersihkan1ActionPerformed(java.awt.event.ActionEvent evt) {                                             
        for(i=0;i<tbUser.getRowCount();i++){
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.dokter_asal_pasien_ranap,user.duta_parkir_rekap_keluar,user.surat_keterangan_layak_terbang,user.bayar_beban_hutang_lain,user.persetujuan_pernyataan_pemilihan_dpjp,user.grafik_harian_pasien_prb,user.pengkajian_resiko_jatuh_timeup from user where user.id_user=AES_ENCRYPT(?,'nur')");//tambah chan 3
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
