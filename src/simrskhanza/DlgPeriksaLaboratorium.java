...
import bridging.koneksiDBSLIMS;//tambah chan
...
private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
    ...
    */
    ...
    private void tampilVANSLAB(String order) { 
        ...
        private void tampilSLIMS(String order) { //tambah chan
        try {
            ...
            for(i2=0;i2<tbTarif.getRowCount();i2++){ 
                if(tbTarif.getValueAt(i2,0).toString().equals("true")){
                    ...
                    try {
                        ...
                        rstampil=pstampil.executeQuery();
                        while(rstampil.next()){
                            pstindakan=koneksislims.prepareStatement("select noorder, kd_jenis_prw, id_template, nilai, nilai_rujukan, keterangan from detail_hasil_lab where noorder=? and id_template=?");
                            try {
                                pstindakan.setString(1,order);
                                pstindakan.setString(2,rstampil.getString("id_template"));
                                rstindakan=pstindakan.executeQuery();
    ...
    public void setOrderSLIMS(String order,String norawat,String posisi){//tambah chan
        noorder=order;
        TNoRw.setText(norawat);
        this.status=posisi;
        isRawat();
        ...
        try {                    
            ...
            if(sukses==true){
                ...
                if(status.equals("Ranap")){
                    ...
                    sukses=jur.simpanJurnal(TNoRw.getText(),"U","K111 PEMERIKSAAN LABORAT RAWAT INAP PASIEN "+TPasien.getText()+" DIPOSTING OLEH "+akses.getkode());  //tambah chan
                }else if(status.equals("Ralan")){
                    Sequel.queryu("delete from tampjurnal");    
                    if(ttlpendapatan>0){
                    ...
                    sukses=jur.simpanJurnal(TNoRw.getText(),"U","K112 PEMERIKSAAN LABORAT RAWAT JALAN PASIEN "+TPasien.getText()+" DIPOSTING OLEH "+akses.getkode()); //tambah chan
                }
            }
